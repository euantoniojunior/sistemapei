...
def verificar_e_criar_colunas():
    """
    Verifica se todas as colunas existem na tabela 'student' e cria se necessário
    """
    engine = db.engine
    ...
    for col_name, col_type in colunas_necessarias.items():
        if col_name not in columns:
            missing_cols.append(f"ADD COLUMN {col_name} {col_type}")

    if missing_cols:
        with engine.connect() as conn:
            for col_sql in missing_cols:
                try:
                    conn.execute(f"ALTER TABLE student {col_sql};")
                    print(f"[INFO] Coluna '{col_sql}' criada na tabela 'student'")
                except Exception as e:
                    print(f"[ERRO] Falha ao criar coluna '{col_sql}': {e}")
            conn.commit()
...
def buscar_aluno_por_id(student_id):
    ...
    if pei:
        try:
            ...
            print(f"[ERRO] Falha ao carregar conteúdo do PEI ID {pei.id}: {e}")
...
def baixar_excel():
    ...
    for aluno in alunos:
        ws.append([
            ...
            aluno.responsavel, aluno.data_nascimento.isoformat() if aluno.data_nascimento else "",
...
# ✅ Função auxiliar: Busca aluno por nome (usada em search.html)
@pei_bp.route('/api/alunos/buscar', methods=['GET'])
...
# ✅ Rota: Listar todos os alunos (registro.html)
